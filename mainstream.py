...
for uploaded_file in uploaded_files:
    ...
    filename.append(name_without_extension(name))
...
def displayPrediction(image_folder):
    anno_postfix = '_annotation.jpg'
    pred_postfix = '_prediction.jpg'
    for i, fn in enumerate(filename):
        annotation = image_folder + '/' + fn + anno_postfix
        prediction = image_folder + '/' + fn + pred_postfix

        cols = st.beta_columns(2)
        cols[0].image(prediction, use_column_width=True,  caption= 'prediction: ' + fn)
        cols[1].image(annotation, use_column_width=True,  caption= 'Annotation: '+fn,)


        # st.image([annotation, prediction], caption=['Annotation: '+fn, 'prediction: ' + fn])

        # get WWR
        wwr = wwr_dictionary[fn]
...
def run_prediction():
    ...
    # Display prediction and WWR
    displayPrediction(prediction_path)
    return
